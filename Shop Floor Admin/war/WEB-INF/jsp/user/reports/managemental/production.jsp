...
function checkValues(){
	...
	}else if(document.productionFrm.costCenter.value == ""){
		alert("Select a CostCenter");
	}else if(document.productionFrm.workCenter.value == ""){
...
document.productionFrm.actionString.value="costCenterListing";
document.productionFrm.submit();
return false;
}

function getWorkCenters(){
document.productionFrm.actionString.value="workCenterListing";
document.productionFrm.submit();
return false;
}
</script>
...
<input type="hidden" name="toexcel"> <br>
<br>
	<CENTER>
...
<TABLE border="0" cellpadding="0" cellspacing="0" width="99%"
	...
	<tr valign="top">
		<td rowspan="4" valign="center" size="5"
			style="font: 900 12px arial;left:20px;top:220px">
			<table width="100%">
						<tr>	<td>Facility</td><td>Cost Center</td><td>Work Center</td></tr>
						<tr>	
						<td>
																					
								<select name="facility" id="facility" onchange="javascript:getCostCenters();" >
									...
									<td>
											<select 	name="costCenter"
															style="width:160px;margin:5px 0 5px 0;font: small-caps 900 10px arial;color:#1A4A6F;"
															size="6" onchange="javascript:getWorkCenters();">
												<c:forEach items="${command.costCenters}" var="data" varStatus="list">
													<option value="${data.costCenterCode}" <c:if test="${command.costCenter eq data.costCenterCode}"> <c:out value="selected"/> </c:if> >
														<c:out value="${data.costCenterCode}"></c:out>-<c:out value="${data.costCenterDesc}"></c:out>	
													</option>
												</c:forEach>						
											</select>
									</td>
									<td>
										<select 	name="workCenter"
											...
											<c:forEach items="${command.workCenters}" var="data" varStatus="list">
													<option value="${data.workcenterCode}" <c:if test="${command.workCenter eq data.workcenterCode}"> <c:out value="selected"/> </c:if>>
														<c:out value="${data.workcenterCode}"></c:out>	-<c:out value="${data.workcenterDesc}"></c:out>
													</option>
												</c:forEach>						
										</select>
							</td>
	...
	<tr valign="top">
		...
		<td><input type="text" name="moNumber" id="moNumber"
			value="${command.moNumber}" onkeyup="upperCase(this.id)"></td>

	</tr>
	<tr valign="top">

		<td style="font: 900 11px arial;left:20px;top:220px">Item #
		</td>
	...
	<tr valign="top">
		...
		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		&nbsp;&nbsp;&nbsp;&nbsp;
			<a href="javascript:checkValues();" alt="Search" tabindex="5" style="cursor: hand;">
							<img src="static/images/search.gif" alt="Search" border="0">
						</a></td>
		</td>
	</tr>
</table>
...
cal2.top = 300;
	...
	<c:when test="${not empty command.workCenter}">

		<c:choose>

			<c:when test='${empty command.productionReport}'>
				...
				<TABLE id="normalTableNostripe" align="center" cellSpacing="0"
					...
					<thead class="fixedHeader">
						<tr class="selected" height="15px">
							...
							<th class="tableTitle" width="6px"><fmt:message
								key="table.pro.column4" /></th>
							<th class="tableTitle" width="6px"><fmt:message
								key="table.pro.column7" /></th>
							<th class="tableTitle" width="12px"><fmt:message 
								key="table.pro.column8" /></th>
							<th class="tableTitle"
						...
						<c:forEach items="${command.productionReport}" var="data"
							...
							<c:choose>
								...
								<td class="tableContent"
									...
									<!-- <td class="tableContent"	remove the comment and commit the above one 
										style="font-weight: bold;width:1300px;white-space: nowrap; word-spacing: normal;"> -->
