...
<head>
<title>Shop Floor - Employee Performance Report</title>
</head>
...
<div id="content">
	...
	<br> <br> 
	<CENTER>
		<font size="2"> <b> <fmt:message key="table.EmployeePerformance.header"/> </b> </font>
	</CENTER>  
 <br> <br>
 <CENTER>
		<font size="2"> <fmt:message key="table.per.column19"/> :<c:out value='${command.empId} - ${command.empName}'></c:out> </font>			
	</CENTER>
...
<c:when test='${empty command.employeePerformanceReport}'>
	...
	<TABLE id="normalTableNostripe" align="center" cellSpacing="0" cellPadding="0" border="0" style="white-space: nowrap; word-spacing: normal;">
			<thead class="fixedHeader">
					<tr class="selected" height="15px">						
						<th class="tableTitle" width="30"> <fmt:message key="table.per.column2"/> </th>
						<th class="tableTitle" width="30"> <fmt:message key="table.per.column3"/> </th>
						<th class="tableTitle" width="30"> <fmt:message key="table.per.column4"/> </th>
						<th class="tableTitle" width="30"> <fmt:message key="table.per.column5"/> </th>
						<th class="tableTitle" width="30"> <fmt:message key="table.per.column6"/> </th>
						<th class="tableTitle" width="30"> <fmt:message key="table.per.column7"/> </th>
						<th class="tableTitle" width="30"> <fmt:message key="table.per.column8"/> </th>
						<th class="tableTitle" width="30"> <fmt:message key="table.per.column9"/> </th>
						<th class="tableTitle" width="30"> <fmt:message key="table.per.column10"/> </th>
						<th class="tableTitle" width="30"> <fmt:message key="table.per.column11"/> </th>
						<th class="tableTitle" width="30"> <fmt:message key="table.per.column12"/> </th>	
						<th class="tableTitle" width="30"> <fmt:message key="table.per.column1"/> </th>					
					</tr>
		</thead>
		<tbody class="scrollContent">
					<c:forEach items="${command.employeePerformanceReport}" var="data" varStatus="list">
					...
					<c:choose>
						...
						<c:otherwise>
							<td class="tableContent" width="100" style="width:4160px;white-space: nowrap; word-spacing: normal;"> <c:out value="${data.assetDesc}"></c:out> </td>
						</c:otherwise>				
					</c:choose>
					...
					<c:choose>					
						...
						<c:otherwise>
							<td class="tableContent" width="30" style="width:1300px;white-space: nowrap; word-spacing: normal;" >  <c:out value="${data.partid}"></c:out> </td>
						</c:otherwise>
					</c:choose>
					...
					<c:choose>
						...
						<c:otherwise>
							<td class="tableContent" width="30" style="font-weight: bold;white-space: nowrap; word-spacing: normal;"> <c:out value="${data.partDesc}"></c:out> </td>
						</c:otherwise>				
					</c:choose>
					...
					<c:choose>
						...
						<c:otherwise>
							...
							<A href="javascript:popupModalWithParams('logonLogoffDetails.htm?empID=${command.empId}&dept=${data.department}&moNumber=${data.moNumber}
							&punchIn=${command.fromDate}&punchOut=${command.toDate}','dialogWidth:900px; dialogHeight:410px; center:yes')"  
