...
public class OutsideOperationReversalActionsHandler extends BaseAjaxHandler {
	...
	private Component buildDetailsTale(ReversalDto dto) {
		...
		TableData warehouseHeader = new TableData(new SimpleText("Warehouse"));
		warehouseHeader.addAttribute("style", getHeaderStyle());
		TableData warehouseData = new TableData(new SimpleText(
				HtmlUtils.htmlEscape(StringUtils.defaultString(dto
						.getWarehouse(), DASH))));
		warehouseData.addAttribute("style", "width: 150px; font-weight: bold");
		row1.addTableData(warehouseHeader);
		row1.addTableData(warehouseData);

		TableData workcenterHeader = new TableData(
		...
		workcenterData.addAttribute("style", "width: 150px; font-weight: bold");
		row1.addTableData(workcenterHeader);
		row1.addTableData(workcenterData);

		TableData poNumberHeader = new TableData(new SimpleText("PO Number"));
		poNumberHeader.addAttribute("style", getHeaderStyle());
		TableData poNumberData = new TableData(
				new SimpleText(HtmlUtils.htmlEscape(StringUtils.defaultString(
						dto.getPoNumber(), DASH))));
		poNumberData.addAttribute("style", "width: 150px; font-weight: bold");
		...
		TableData poLineData = new TableData(new SimpleText(HtmlUtils
				.htmlEscape(StringUtils.defaultString(dto.getPoLineNumber(),
						DASH))));
		...
		if (sequences != null && sequences.size() > 0) {
			...
			for (Sequence sequence : sequences) {
				...
				TableData employeeData = new TableData(new SimpleText(HtmlUtils
						.htmlEscape(StringUtils.defaultString(sequence
								.getEmployeeName(), DASH))));
