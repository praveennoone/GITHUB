 ...
 * 
 */
public class LogOnOffExcelView extends AbstractJExcelView {

	private Map model = null;

	public LogOnOffExcelView(Map model) {
	...
	protected void buildExcelDocument(Map model,
		...
		try {
			if (reports != null) {
				...
				label = new Label(6, 0, "MO Number");
				sheet.addCell(label);

				label = new Label(7, 0, "Line Number");
				sheet.addCell(label);

				label = new Label(8, 0, "Sequence");
				sheet.addCell(label);

				label = new Label(9, 0, "Activity");
				sheet.addCell(label);

				label = new Label(10, 0, "Logon Time");
				sheet.addCell(label);

				label = new Label(11, 0, "Logoff Time");
				sheet.addCell(label);
				...
				for (int i = 0, j = 2; i < reports.size(); i++, j++) {
					...
					sheet.addCell(label);
					label = new Label(6, j, report.getMoNumber());
					sheet.addCell(label);
					label = new Label(7, j, report.getLineNumber());
					sheet.addCell(label);
					label = new Label(8, j, report.getSequence());
					sheet.addCell(label);
					label = new Label(9, j, report.getActivity());
					sheet.addCell(label);
					label = new Label(10, j, report.getLogon());
					sheet.addCell(label);
					label = new Label(11, j, report.getLogoff());
					sheet.addCell(label);
